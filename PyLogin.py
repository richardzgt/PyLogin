...
def UpdateDB(cmd,DBValue=0):
    ...
    if cmd == 'QryAuth':
        SQL='''select b.IP,a.IPiroot from  t_UserAuth a,t_HostPasswd b where 
        a.UserID=%s and b.ID=a.HPID  ''' % MyLdap.ldap_get_user(DBValue)['uid']
        cursor.execute(SQL)
        try:
    ...
    if cmd == 'ModPass':
        if  MyLdap.ldap_update_pass(Value['User'],Value['Passwd'],DBValue):
            help(21)
        else:
            help(22)
...
def GetUserAndPri():
    ...
    if Value['User'] and Value['Passwd']:
        ...
        try:
            UserID = MyLdap.ldap_get_user(Value['User'])['uid']
        except Exception,e:
            help(7)
            sys.exit()
        ...
        #校验用户权限
        if MyLdap.ldap_get_vaild(Value['User'],Value['Passwd']) :
            try:
                conn = MySQLdb.connect(ConF['host'],ConF['user'],ConF['passwd'],ConF['db'],port=3306,charset='utf8')
            except Exception,e:
...
#===============================================================================
    ...
    if Value['ForkCmd'] == '-l' and MyLdap.ldap_get_vaild(Value['User'],Value['Passwd']):
        Menu()
